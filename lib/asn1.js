...
function encodeBufferAsInteger(valueBuffer) {
    ...
    // Trim all leading zero bytes if length > 1 (otherwise we trim away the value '0')
    if (valueBuffer.length > 1) {
        ...
        if (firstNonZeroPos != 0) {
            valueBuffer = valueBuffer.slice(firstNonZeroPos);
        }
    }

    // We need to pad the buffer with a leading zero if the first byte is greater than 0x7f.
    if (valueBuffer[0] > 0x7f) {
        valueBuffer = Buffer.concat([ new Buffer([ 0 ]), valueBuffer ]);
    }
