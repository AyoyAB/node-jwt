...
exports.encodeObjectIdentifier = function(valueBuffer) {
    ...
    return Buffer.concat([ new Buffer([ TAG ]), exports.encodeLengthBytes(valueBuffer.length), valueBuffer ]);
};
...
exports.encodeSequence = function(valueBufferArray) {
    var TAG = 0x30, contentBytes;

    contentBytes = Buffer.concat(valueBufferArray);

    return Buffer.concat([ new Buffer([ TAG ]), exports.encodeLengthBytes(contentBytes.length), contentBytes ]);
};
