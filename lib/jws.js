...
var getMacAlgorithm = function(algorithm) {
    switch (algorithm) {
        ...
        case exports.algorithm.EcdsaP256WithSha256:
 ...
 */
exports.createSignature = function(algorithm, key, protectedHeader, payload) {
    var macAlgorithm = getMacAlgorithm(algorithm);
    var keyBuffer = jwk.jwkToOpenSSL(key);
    var payloadBuffer = new Buffer(protectedHeader + '.' + payload);
    return base64url.fromBase64String(pubKey.doSign(macAlgorithm, keyBuffer, payloadBuffer).toString('base64'));
};
 ...
 */
exports.validateSignature = function(algorithm, key, protectedHeader, payload, signature) {
    var macAlgorithm = getMacAlgorithm(algorithm);
    var keyBuffer = jwk.jwkToOpenSSL(key);
    var payloadBuffer = new Buffer(protectedHeader + '.' + payload);
