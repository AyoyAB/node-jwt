...
signatureAlgorithm = {
    ...
    /** HMAC with SHA-384 */
    HmacWithSha384: 'HS384',
    /** HMAC with SHA-512 */
    HmacWithSha512: 'HS512',
    /** ECDSA with Suite B elliptic curve P-256 and SHA-256 */
    EcdsaP256WithSha256: 'ES256',
    /** ECDSA with Suite B elliptic curve P-384 and SHA-384 */
...
getMacAlgorithm = function(algorithm) {
    switch (algorithm) {
        ...
        case signatureAlgorithm.HmacWithSha384:
        case signatureAlgorithm.EcdsaP384WithSha384:
            return hmac.algorithm['HMAC-SHA384'];
        case signatureAlgorithm.HmacWithSha512:
...
/**
 * Checks if a signing algorithm is an HMAC algorithm.
 *
 * @function isHmacAlgorithm
 *
 * @param {signatureAlgorithm} algorithm - A JWA signing algorithm
 *
 * @returns {boolean} True if the algorithm is a HMAC algorithm
 */
isHmacAlgorithm = function(algorithm) {
    switch (algorithm) {
        case signatureAlgorithm.HmacWithSha256:
        case signatureAlgorithm.HmacWithSha384:
        case signatureAlgorithm.HmacWithSha512:
...
validateSignature = function(algorithm, key, protectedHeader, payload, signature) {
    var macAlgorithm = getMacAlgorithm(algorithm);
    var keyBuffer = jwk.jwkToOpenSSL(key, { 'public': true });
    var payloadBuffer = new Buffer(protectedHeader + '.' + payload);
...
module.exports = {
    isEcdsaAlgorithm: isEcdsaAlgorithm,
    isHmacAlgorithm: isHmacAlgorithm,
