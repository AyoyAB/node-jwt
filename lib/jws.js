...
/**
 * Trims the input integer Buffer to the expected length.
 *
 * This is used to trim away leading zeroes added during ASN.1 encoding.
 *
 * @function trimIntegerBuffer
 *
 ...
 */
trimIntegerBuffer = function(intBuffer, requestedLength) {
    var offset, targetBuffer;
    if (requestedLength > intBuffer.length) {
        throw new Error('Requested length ' + requestedLength + ' larger than buffer size: ' + intBuffer.length);
...
convertEcdsaSignatureToJws = function(ecdsaSignature, algorithm) {
    var decodedSignature = asn1.decodeDerEcdsaSignature(ecdsaSignature),
        r = trimIntegerBuffer(decodedSignature.r, getSignatureParameterLength(algorithm)),
        s = trimIntegerBuffer(decodedSignature.s, getSignatureParameterLength(algorithm));
...
convertJwsSignatureToEcdsa = function(jwsSignature, algorithm) {
    ...
    if (jwsSignature.length != 2 * paramLen) {
        throw new Error('Input length not equal to double algorithm length: ' + paramLen);
    }
