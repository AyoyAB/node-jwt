 ...
 * @param {Boolean} [opt.public] Explicitly output only a public key from contents
 *
 * @returns {Buffer} The key in OpenSSL format
 */
exports.jwkToOpenSSL = function (jwk, opt) {
    var namedCurve;
    ...
    switch (jwk.kty) {
        ...
        case 'EC':
            ...
            if (!jwk.d || (opt && opt.public)) {
                // No private key (d) present. Encode as public key.
                // Optionally, the caller has explicitly selected only public key output.
