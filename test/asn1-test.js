...
describe('asn1', function() {
    ...
    describe('.encodeECPrivateKey()', function () {
        var TEST_P256_KEY = {
            ...
            }, TEST_256_KEY_ENCODING = Buffer.concat([
                new Buffer([ 0x30, 0x77, 0x02, 0x01, 0x01, 0x04, 0x20 ]),
                TEST_P256_KEY.d,
                new Buffer([ 0xa0, 0x0a, 0x06, 0x08, 0x2a, 0x86, 0x48, 0xce, 0x3d, 0x03, 0x01, 0x07, 0xa1, 0x44, 0x03, 0x42, 0x00, 0x04 ]),
                TEST_P256_KEY.x,
                TEST_P256_KEY.y
            ]);

        it('should encode TEST_P256_KEY correctly', function () {
            var res = asn1.encodeECPrivateKey(TEST_P256_KEY.curve, TEST_P256_KEY.d, TEST_P256_KEY.x, TEST_P256_KEY.y);
            expect(bufferEquals(res, TEST_256_KEY_ENCODING)).to.equal(true);
        });
    });
});
